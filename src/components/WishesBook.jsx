        ...
        <div>
            <div className="card lg:card-side shadow-xl border p-6">
                ...
                <div className="card-body lg:space-y-4 px-0 lg:px-8 lg:py-0">
                    ...
                    <span>{author}</span>
                    <ul className="flex flex-col lg:flex-row gap-2 lg:gap-6 text-[#23BE0A] font-medium">
                        <li className="text-black font-bold">Tag: </li>
                        {
                            tags.map((tag, idx) => <li key={idx}>#{tag}</li>)
                        }
                        <li className="flex items-center gap-3 text-[#131313CC]"><CiLocationOn /> Year of Publishing: {yearOfPublishing}</li>
                    </ul>
